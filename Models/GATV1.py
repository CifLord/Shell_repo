...
@registry.register_model('GATV1')
class GATV1(GemNetOC):
    """
    Arguments
    ---------
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        self.gat_layers= GATlayer(c_in=emb_size_in,c_out=emb_size_in,num_heads=num_heads)
    ...
    def forward(self, data):
        ...
        # Graph attention part
        E_t = self.gat_layers(h,main_graph['edge_index'])
        #print(checkpoint_)
        print(E_t.shape)
        #(E_t.shape==n_batch x 256 x 1)
        #E_t = E_t.permute(0,2,1)
        
        E_t = self.dense(E_t+h)

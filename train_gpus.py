...
def main(rank,world_size):
    ...
    train_loader = DistributedDataLoader(train_dataset, batch_size=batch_size,rank=rank,world_size=world_size,drop_last=True)
    val_loader =DistributedDataLoader(val_dataset, batch_size=batch_size,rank=rank,world_size=world_size,drop_last=True)
    
    best_valid_loss=np.Inf
    for epoch in range(num_epochs):
        
        train_loss=train_fn(train_loader,model,optimizer,device=rank)
        valid_loss,acc=eval_fn(val_loader,model,device=rank)

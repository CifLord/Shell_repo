...
class SlabEntry(ComputedStructureEntry):
    ...
    def __init__(
        ...
        self.mark = marker
        self.color = color
        self.preset_surface_energy = preset_surface_energy
    ...
    def surface_energy(self, ucell_entry, ref_entries=None, referenced=True):
        ...
        if self.preset_surface_energy:
            return self.preset_surface_energy
        ...
        # assumes all refs are elemental
        ...
        for el, ref in ref_entries_dict.items():
            if referenced:
                N, u = self.composition.as_dict()[el], Symbol("delu_" + str(el))
            else:
                N, u = self.composition.as_dict()[el], Symbol("u_" + str(el))

            if el in ucell_comp.as_dict():
                if referenced:
                    gbulk_eqn += ucell_reduced_comp[el] * (u + ref.energy_per_atom)
                else:
                    gbulk_eqn += ucell_reduced_comp[el] * u
                    
            if referenced:
                bulk_energy += N * (Symbol("delu_" + el) + ref.energy_per_atom)
            else:
                bulk_energy += N * (Symbol("u_" + el))
...
class SurfaceEnergyPlotter:
    ...
    def wulff_from_chempot(
        ...
        get_conventional_standard_structure().lattice if not latt else latt

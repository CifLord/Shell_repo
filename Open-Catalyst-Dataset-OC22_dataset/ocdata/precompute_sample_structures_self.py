...
def enumerate_surfaces_for_saving(bulk_atoms, max_miller=MAX_MILLER,layers=4):
    '''
    ...
    bulk_ase=AseAtomsAdaptor.get_atoms(bulk_struct)
    ...
    for millers in get_symmetrically_distinct_miller_indices(bulk_struct, MAX_MILLER):
        ...
        #                            max_broken_bonds=0,
        #                            symmetrize=False)
        slabs1=surfaces_with_termination(bulk_ase,indices=millers,layers=layers,vacuum=20)
        slabs=[]
        for slab in slabs1:
            slabs.append(AseAtomsAdaptor.get_structure(slab))
        

        # If the bottoms of the slabs are different than the tops, then we want
        # to consider them, too
        shift=0
        flipped_slabs_info = [(flip_struct(slab), millers,shift, False)
                              for slab in slabs if is_structure_invertible(slab) is False]

        # Concatenate all the results together
        slabs_info = [(slab, millers,shift, True) for slab in slabs]

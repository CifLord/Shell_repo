...
def read_options():
    ...
    parser.add_argument("-t", "--nthreads", dest="number_of_threads", type=int, default=3,
                        help="Number of threads to distribute predictions to")
    parser.add_argument("-h", "--mmi", dest="mmi", type=int, default=1,
...
if __name__=="__main__":
    ...
    for i in range(args.nthreads):
        lp = range(int(len(input_lmdb)/number_of_threads)*i, int(len(input_lmdb)/number_of_threads)*(1+i))         
